...
{
    // Инициализация контроллеров всех специализаций
    ...
    // Привязка колбек функции, которая связывает вью с контроллером конкретной специализации
    ...
    // Дефолтное состояние - привязка вью в double контроллеру
    double_controller.BindWithMainWindow(&w);
    w.show();
    return a.exec();
}
